...
from config import VOLC_AUDIO_PARAMS, VOLC_APP_ID, VOLC_SK, VOLC_HOST, VOLC_RESOURCE_ID
...
SPEAKER_MEILI_NVYOU = "zh_female_roumeinvyou_emo_v2_mars_bigtts"
...
audio_params = VOLC_AUDIO_PARAMS
        ...
        try:
            ...
            while (not self._exit.is_set()):
                ...
                if msg.type == MsgType.AUDIO_ONLY_SERVER:
                    await self.audio_callback(msg.payload)
                elif msg.type == MsgType.FULL_SERVER:
                    if msg.event == Event.EVENT_SESSION_FINISHED:
                        break
                elif msg.type == MsgType.ERROR:
                    print(f"Error received: {msg.payload}")
                    break
        finally:
            # 接收结束后不再关心发送
            if sender_task:
